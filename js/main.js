import { initUI, renderParamsUI, elements, triggerHaptic, triggerHapticError, renderNumberPad } from "./ui.js";
...
function copyToClipboard(text) {
  if (navigator.clipboard && window.isSecureContext) {
    navigator.clipboard.writeText(text).then(() => {
      ...
      setTimeout(() => (elements.copyBtn.textContent = "Copy as Markdown"), 2000);
    });
  } else {
    // Fallback
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  attach(elements.closeHelpBtn, () => elements.helpModal.classList.remove("visible"));
