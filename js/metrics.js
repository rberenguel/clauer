...
export function calculateMetrics(logs, totalTimeSec) {
  ...
  // New Logic: 
  const timedCorrectCount = (logs.length > 0 && logs[0].correct) ? correctLogs.length - 1 : correctLogs.length; 
  ...
  const cpm = minutes > 0 ? (Math.max(0, timedCorrectCount) / minutes).toFixed(1) : 0;
  ...
  if (n > 1) {
    ...
    const variance = responseTimes.reduce((a, b) => a + Math.pow(b - meanRT, 2), 0) / n;
  ...
  if (switchLogs.length > 0 && stableLogs.length > 0) {
    avgSwitchRT = switchLogs.reduce((a, b) => a + b.responseTime, 0) / switchLogs.length;
    avgStableRT = stableLogs.reduce((a, b) => a + b.responseTime, 0) / stableLogs.length;
...
export function generateRecoveryGraphData(logs, batchSize) {
  ...
  logs.filter(l => l.correct).forEach(log => {
  ...
  for (let i = 0; i < maxPos; i++) {
      ...
      if (rtsByPosition[i] && rtsByPosition[i].length > 0) {
          const avg = rtsByPosition[i].reduce((a, b) => a + b, 0) / rtsByPosition[i].length;
